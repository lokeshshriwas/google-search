...
const SearchBox = () => {
  ...
  return (
    <form className="flex border border-stone-600 rounded-full px-6 py-3 ml-10 me-5 flex-grow max-w-3xl items-center" onSubmit={handleSubmit}>
      ...
      />
      <RxCross2 className="text-2xl text-gray-300 cursor-pointer sm:mr-2" onClick={()=>setTerm("")}/>
      <FaMicrophone className="hidden sm:inline-flex text-4xl text-gray-200 border-l-2 border-stone-600 mr-3 pl-4" />
      <CiSearch className="text-3xl hidden sm:inline-flex text-gray-200 cursor-pointer" onClick={handleSubmit}/>
    </form>
  );
};
